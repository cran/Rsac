...
static complex cmul(), cpowi(), csqrt(), conjg(), cdiv();
...
/* FUNC DEF */ static complex csqrt(z)
complex z;
{	complex c;
	float x,y,w,r;
	if ((z.r == 0.0) && (z.i == 0.0)) {
...
/*   older test stuff
double  jladd(double  *a, double *b, double   *ans)
{
...
*/
/** FUNC DEF */ void jchar(char **type, char **aproto)
{
...
/** FUNC DEF */ void jfilt(float *input, int *na, int *iord, int *zp, char **type, char **aproto,
...
apply(why, k, *zp);
  /*  1 = true for zero phase filtering   */
  /*  0 = false for single pass filtering */
